...
public class BoardTestSuite {
    ...
    public void testAddTaskListAverageWorkingOnTask(){
        ...
        List<TaskList> inProgressTasks = new ArrayList<>();
        inProgressTasks.add(new TaskList("In progress"));
        long sumCreated = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(t -> t.getCreated())
                .map(n -> DAYS.between(n, LocalDate.now()))
                .reduce((long) 0, (sum, current) -> sum += current);

        long countCreated = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(t -> t.getCreated())
                .count();

        long averageCreated = sumCreated / countCreated;

        //Then
        Assert.assertEquals(10, averageCreated);
    }
}
